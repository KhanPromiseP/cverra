...
export const BlockSection = () => {
  ...
  // Initialize local content when selected block changes
  useEffect(() => {
    if (selectedBlockData?.content !== undefined) {
      setLocalContent(selectedBlockData.content || '');
      setIsTyping(false);
    }
  ...
  // Debounced content update with immediate local state
  const handleContentUpdate = (newContent: string) => {
    ...
    autoSaveTimeoutRef.current = setTimeout(() => {
      console.log('⏰ Sidebar auto-saving content', {
        blockId: selectedBlock,
        contentLength: newContent.length
      });
      updateBlock(selectedBlock, { content: newContent });
      setIsTyping(false);
  ...
  const handleImmediateSave = () => {
    if (!selectedBlock || !localContent) return;
    ...
    console.log('💾 Sidebar force saving content');
    updateBlock(selectedBlock, { content: localContent });
    setIsTyping(false);
  ...
  const handleEnhanceBlock = async () => {
    ...
    try {
      ...
      useCoverLetterStore.getState().setCoverLetter(result.coverLetter);
      setEnhanceInstructions('');
      toast.success(t`Block enhanced successfully!`);
    } catch (error) {
      console.error(t`Enhancement failed:`, error);
      toast.error(t`Failed to enhance block`);
    } finally {
      setIsEnhancing(false);
    }
  };
  ...
  const handleRegenerateBlock = async () => {
    ...
    try {
      const result = await coverLetterService.regenerateBlock(coverLetter.id, selectedBlock);
      useCoverLetterStore.getState().setCoverLetter(result.coverLetter);
      toast.success(t`Block regenerated successfully!`);
    } catch (error) {
      console.error(t`Regeneration failed:`, error);
      toast.error(t`Failed to regenerate block`);
    } finally {
      setIsEnhancing(false);
    }
  };
  ...
  const fontSizes = [
    { value: '10px', label: t`XS (10px)` },
    { value: '12px', label: t`Small (12px)` },
    { value: '14px', label: t`Normal (14px)` },
    { value: '16px', label: t`Large (16px)` },
    { value: '18px', label: t`XL (18px)` },
    { value: '20px', label: t`XXL (20px)` },
    { value: '24px', label: t`Heading (24px)` },
    { value: '28px', label: t`Title (28px)` }
  ];

  const lineHeights = [
    { value: '1.0', label: t`Tight (1.0)` },
    { value: '1.2', label: t`Compact (1.2)` },
    { value: '1.4', label: t`Normal (1.4)` },
    { value: '1.5', label: t`Relaxed (1.5)` },
    { value: '1.6', label: t`Loose (1.6)` },
    { value: '1.8', label: t`Spacious (1.8)` },
  ...
  const alignments = [
    ...
    { value: 'justify', label: t`Justify`, icon: AlignJustify }
  ];

  const fontFamilies = [
    { value: 'inherit', label: t`System Default` },
    { value: 'Arial, sans-serif', label: t`Arial` },
    { value: 'Georgia, serif', label: t`Georgia` },
    { value: "'Times New Roman', serif", label: t`Times New Roman` },
    { value: "'Helvetica Neue', sans-serif", label: t`Helvetica` },
    { value: "'Garamond', serif", label: t`Garamond` },
    { value: "'Verdana', sans-serif", label: t`Verdana` },
    { value: "'Courier New', monospace", label: t`Courier New` }
  ];

  const fontWeightOptions = [
    { value: '100', label: t`Thin` },
    { value: '200', label: t`Extra Light` },
    { value: '300', label: t`Light` },
    { value: '400', label: t`Normal` },
    { value: '500', label: t`Medium` },
    { value: '600', label: t`Semi Bold` },
    { value: '700', label: t`Bold` },
    { value: '800', label: t`Extra Bold` },
    { value: '900', label: t`Black` }
  ];

  const borderRadiusOptions = [
    { value: '0px', label: t`None` },
    { value: '2px', label: t`Small` },
    { value: '4px', label: t`Medium` },
    { value: '8px', label: t`Large` },
    { value: '12px', label: t`XL` },
    { value: '16px', label: t`XXL` },
    { value: '24px', label: t`Round` },
    { value: '50%', label: t`Circle` }
  ];
    ...
    <section id="block" className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          {isTyping && (
            <div className="flex items-center space-x-1 px-2 py-1 text-xs bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded">
      ...
      <div className="space-y-3">
        ...
        />
        <div className="flex justify-between text-xs text-gray-500">
          <span>{localContent.length || 0} {t`characters`}</span>
          <span className="flex items-center gap-2">
            <span className="text-xs text-gray-400">{t`Supports **bold**, *italic*, _underline_`}</span>
            <span className="text-xs text-blue-500">• {t`Ctrl+Enter: Save • Esc: Revert`}</span>
          </span>
        </div>
      </div>
