...
export const LocaleCombobox = ({ value, onValueChange }: Props) => {
  ...
  return (
    <Command shouldFilter={false}>
      {/* Removed CommandInput for search */}
      <CommandList>
          ...
          <ScrollArea orientation="vertical">
            <div className="max-h-60">
              {languages.map((lang) => (
                <CommandItem
                  key={lang.locale}
                  value={lang.locale.trim()}
                  onSelect={() => onValueChange(lang.locale)}
                >
                  <Check
                    className={cn(
                      "mr-2 size-4 opacity-0",
                      value === lang.locale && "opacity-100"
                    )}
                  />
                  {lang.name}{" "}
                  <span className="ml-1 text-xs opacity-50">({lang.locale})</span>
                </CommandItem>
              ))}
            </div>
          </ScrollArea>
