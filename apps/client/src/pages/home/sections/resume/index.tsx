...
import { ArrowRight, Sparkle, FileText, Palette, Globe, MagicWand, Download, Eye, Layout, Flask } from "@phosphor-icons/react";
import { buttonVariants, Badge, Button } from "@reactive-resume/ui";
...
export const ResumeSection = ({ onViewTemplates }: ResumeSectionProps) => {
  // Import the duplicateResume function to create sample resumes
  ...
  const onCreateSample = async () => {
    ...
    try {
      ...
      console.error("Failed to create sample resume:", error);
    }
  };
        ...
        >
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 lg:p-12 text-white shadow-xl sm:shadow-2xl">
            ...
            <p className="text-blue-100 text-sm sm:text-base md:text-lg mb-6 sm:mb-8 max-w-2xl mx-auto leading-relaxed">
              {t`Join thousands of professionals who landed their dream jobs with Inlirah-powered resumes.`}
            </p>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-stretch sm:items-center">
              ...
              <Button
                ...
                className="bg-white/20 hover:bg-white/30 text-white font-semibold px-6 sm:px-8 py-3 rounded-xl sm:rounded-2xl border border-white/30 transition-all duration-300 w-full sm:w-auto flex items-center justify-center gap-2"
              >
                <Flask className="w-4 h-4 sm:w-5 sm:h-5" />
                {duplicateLoading ? t`Creating...` : t`Try Sample Resume`}
              </Button>
              <button 
