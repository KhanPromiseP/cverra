...
export const ResumeCard = ({ resume }: Props) => {
  ...
  console.log('Opening resume:', {
    id: resume.id,
    title: resume.title,
    isAI: (resume.data as any)?.metadata?.aiGenerated,
  });
  
  const isAI = (resume.data as any)?.metadata?.aiGenerated;
...
const repairAIResume = async () => {
  try {
    toast({
      title: "Optimizing Resume",
      description: "Preparing AI resume for editing...",
      variant: "default",
    });

    const response = await fetch(`/api/resume/${resume.id}/repair`, {
      method: 'PATCH',
      headers: { 
    ...
    if (response.ok) {
      toast({
        title: "Ready!",
        description: "AI resume optimized successfully.",
        variant: "success",
      });
      
      // Refresh and open
      setTimeout(() => navigate(`/builder/${resume.id}`), 500);
    } else {
      throw new Error('Optimization failed');
    }
  } catch (error) {
    console.error('Failed to optimize AI resume:', error);
    
    // Still try to open it
    toast({
      title: "Opening Resume",
      description: "Opening with basic compatibility mode...",
      variant: "info",
    });
        ...
        >
          ...
          {!isTranslation && (
            <div className="absolute top-60 left-3 z-30">
              <Badge 
                variant="outline" 
                className="bg-blue-500/20 backdrop-blur-sm border-blue-400 ext-black dark:text-white text-xs font-medium shadow-sm"
              >
                <Globe size={10} className="mr-1" />
                {t`Original`}
              </Badge>
...
//   const isAIResume = resumeMetadata?.aiGenerated === true || 
//                      resumeMetadata?.generatedBy === 'ai' ||
//                      resume.data?.metadata?.aiGenerated === true ||
//                      (resume as any).metadata?.aiGenerated === true;
...
//         // If validation fails, repair the resume
//         await repairAIResume();
//         return;
//       }
...
//     } catch (error) {
//       console.error('Failed to open AI resume:', error);
//       toast({
//         title: t`Error Opening Resume`,
...
//   try {
//     toast({
//       title: t`Repairing Resume`,
//       description: t`Fixing AI resume structure...`,
//       variant: "default",
//     });

//     const response = await fetch(`/api/resume/repair/${resume.id}`, {
//       method: 'POST',
...
//     if (response.ok) {
//       toast({
//         title: t`Resume Repaired`,
//         description: t`Opening now...`,
//         variant: "success",
//       });
      
//       setTimeout(() => navigate(`/builder/${resume.id}`), 1000);
//     } else {
//       throw new Error('Repair failed');
//     }
//   } catch (error) {
//     console.error('Failed to repair AI resume:', error);
//     toast({
//       title: t`Repair Failed`,
//       description: t`Could not repair the resume. Please contact support.`,
//       variant: "error",
//     });
//   }
// };
...
//               <Badge 
//                 variant="outline" 
//                 className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 backdrop-blur-sm border-purple-400 text-purple-700 dark:text-purple-300 text-xs font-medium shadow-sm"
//               >
...
//               <Badge 
//                 variant="outline" 
//                 className="bg-blue-500/20 backdrop-blur-sm border-blue-400 text-blue-700 dark:text-blue-400 text-xs font-medium shadow-sm"
//               >
//                 <Globe size={10} className="mr-1" />
//                 {t`Original`}
//               </Badge>
...
//                   <span className="font-medium">{t`Open`}</span>
//                   {isAIResume && (
//                     <Badge variant="outline" className="ml-2 text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
//                       AI
//                     </Badge>
//                   )}
//                 </DropdownMenuItem>
...
//           <div className="relative aspect-[3/4] overflow-hidden rounded-sm">
//             {isAIResume && (
//               <div className="absolute inset-0 z-10 bg-gradient-to-br from-purple-500/10 to-pink-500/10" />
//             )}
//             <img
//               src={`/templates/jpg/resumes/${template}.jpg`}
//               alt={template}
...
//             {/* AI overlay effect */}
//             {isAIResume && (
//               <div className="absolute inset-0 bg-gradient-to-t from-purple-500/10 to-transparent opacity-50 group-hover:opacity-70 transition-opacity duration-300" />
//             )}
//           </div>
//         </BaseCard>
//       </div>
