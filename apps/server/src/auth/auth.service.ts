...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<UserWithSecrets> {
    ...
    try {
      ...
      // Send verification email with the new template
      void this.sendVerificationEmail(user.email);
  ...
  async forgotPassword(email: string) {
    try {
      ...
      if (!user) {
        // Log but don't throw to prevent email enumeration
  ...
  try {
    ...
    const user = await this.userService.findOneByEmail(email);
    if (!user) {
      Logger.error(`Verification email requested for non-existent user: ${email}`);
      return;
    }
    ...
    try {
      ...
      Logger.log(`Verification email sent to ${email}`, "AuthService");
    } catch (mailError) {
      Logger.warn(`Could not send verification email to ${email}. Check SMTP configuration.`, "AuthService");
      // Don't throw, just log - the user can still request a new email
    }
  } catch (error) {
    Logger.error(`Failed to process verification email for ${email}:`, error);
  }
}
