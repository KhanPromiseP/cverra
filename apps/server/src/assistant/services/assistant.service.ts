...
//     private futureSimulationService: FutureSimulationService,
...
//     this.logger.debug(`[${requestId}] sendMessage called:`, {
//       userId,
...
//     // 🚀 STEP 1: Check rate limits (fast, cached)
//     await this.checkRateLimits(userId, userTier);

//     // 🚀 STEP 2: Get or create conversation (fast, indexed)
...
//     // ✅ FIX: Remove 'metadata' field if it doesn't exist in schema
...
//         // metadata removed - add to your schema if needed
//       },
//     });

//     // 🚀 STEP 4: ADD TO MESSAGE BUFFER (async - don't await)
//     // ✅ FIX: Added proper typing for error
...
//       timestamp: new Date(),
//       intent: null,
//     }).catch((err: Error) => this.logger.error('Buffer add failed:', err));

//     // 🚀 STEP 5: Get cached context or build fresh (parallel)
//     const [cachedContext, recentMessages] = await Promise.all([
//       this.cacheService.getConversationContext(conversation.id),
//       this.prisma.assistantMessage.findMany({
//         where: { conversationId: conversation.id },
//         orderBy: { createdAt: 'desc' },
//         take: this.getContextSizeByTier(userTier),
//       }),
//     ]);

//     // Build context (if not cached)
//     let fullContext = cachedContext;
//     if (!fullContext) {
//       const [userContext, contentContext, careerContext, relevantMemories] = await Promise.all([
//         this.contextBuilder.buildUserContext(userId),
//         this.contextBuilder.buildContentContext(dto.contextIds),
//         this.contextBuilder.buildCareerContext(userId),
//         this.getRelevantMemoriesWithCache(userId, dto.content, 3),
//       ]);
...
//       // Cache for future use (don't await)
//       this.cacheService.cacheConversationContext(conversation.id, fullContext)
//         .catch((err: Error) => this.logger.error('Context caching failed:', err));
//     }

//     // 🚀 STEP 6: Prepare messages for AI (fast)
...
//     // 🚀 STEP 7: Get AI response (the only blocking API call)
...
//     // 🚀 STEP 9: Update conversation (fast)
...
//     // 🚀 STEP 10: ASYNC POST-PROCESSING (don't await - fire and forget)
...
//       aiResponse, 
//       userTier
//     ).catch((err: Error) => this.logger.error('Post-processing failed:', err));

//     // 🚀 STEP 11: Prepare response (fast)
//     const responseTime = Date.now() - startTime;
//     const referencedArticles = await this.extractReferencedArticles(aiResponse.content);
//     const rateLimit = await this.getRateLimitInfo(userId, userTier);

//     this.logger.debug(`[${requestId}] Response sent in ${responseTime}ms`);
...
//   /**
//    * Run all post-processing tasks asynchronously
//    */
...
//   ): Promise<void> {
//     const startTime = Date.now();
//     const processingId = this.generateRequestId();
    
//     this.logger.debug(`[${processingId}] Starting post-processing`);

//     // Run all tasks in parallel for maximum efficiency
//     await Promise.allSettled([
//       this.updateAnalytics(userId, aiResponse.tokens, dto.content, aiResponse.metadata?.intent),
//       this.cacheArticleReferences(userId, aiResponse.content),
//       this.cacheService.clearPattern(`conversation:${conversationId}:*`),
//       this.checkEmotionalIntervention(userId, conversationId, messageId, dto.content, aiResponse),
//       this.checkDecisionQuery(userId, messageId, dto.content, aiResponse),
//       this.checkSimulationQuery(userId, messageId, dto.content, userTier, aiResponse),
//     ]);

//     this.logger.debug(`[${processingId}] Post-processing completed in ${Date.now() - startTime}ms`);
//   }

//   /**
//    * Check if emotional intervention is needed
//    */
//   private async checkEmotionalIntervention(
//     userId: string,
//     conversationId: string,
//     messageId: string,
//     content: string,
//     aiResponse: any,
//   ): Promise<void> {
//     const emotionalKeywords = ['sad', 'angry', 'frustrated', 'overwhelmed', 'stressed', 'anxious'];
//     const lowerContent = content.toLowerCase();
    
//     if (!emotionalKeywords.some(k => lowerContent.includes(k))) {
//       return;
//     }

//     try {
//       const analysis = await this.emotionalIntelligenceService.analyzeEmotionalState(
//         userId,
//         content,
//         conversationId,
//         messageId,
...
//       if (analysis && analysis.intensity >= 7) {
//         const intervention = await this.emotionalIntelligenceService.generateEmotionalIntervention(
//           userId,
//           analysis,
//         );

//         if (intervention) {
//           // ✅ FIX: Use correct field names based on your schema
//           await this.prisma.assistantMemory.create({
//             data: {
//               userId,
//               topic: 'emotional_intervention',
//               summary: intervention,
//               contextType: 'emotional_support',
//               importance: 'HIGH',
//               source: 'system',
//               tags: ['emotional', 'intervention', analysis.primaryState.toLowerCase()],
//               lastAccessed: new Date(),
//               // ✅ FIX: Store metadata as JSON string or object
//               metadata: {
//                 messageId,
//                 emotionalState: analysis.primaryState,
...
//       }
//     } catch (error) {
//       this.logger.error('Emotional intervention check failed:', error);
//     }
//   }

//   /**
//  * Check if this is a decision query
//  */
// private async checkDecisionQuery(
//   userId: string,
//   messageId: string,
//   content: string,
//   aiResponse: any,
...
//     /help me decide/i,
//     /should i (?:choose|pick|go with|take)/i,
//     /which (?:is better|should i choose)/i,
//     /compare (.*?) and (.*?)/i,
//   ];
...
//   try {
//     const options = await this.extractDecisionOptions(content);
//     if (options.optionA && options.optionB) {
//       const decisionAnalysis = await this.decisionEngineService.analyzeDecision(
//         userId,
//         content,
//         options.optionA,
//         options.optionB,
...
//         userId,
//         context: content,
//         options: [options.optionA, options.optionB],
//         recommendation: decisionAnalysis.recommendation || null,
//         confidence: decisionAnalysis.confidence || 0,
//       };

//       // Only add scores if they exist and are JSON-safe
//       if (decisionAnalysis.scores) {
//         data.scores = JSON.parse(JSON.stringify(decisionAnalysis.scores));
//       }

//       // Add goalAlignment if it exists in your schema
//       // if (decisionAnalysis.goalAlignment) {
//       //   data.goalAlignment = JSON.parse(JSON.stringify(decisionAnalysis.goalAlignment));
//       // }

      

//       await this.prisma.assistantDecision.create({ data });
//     }
//   } catch (error) {
//     this.logger.error('Decision query check failed:', error);
//   }
// }
...
//  */
// private async checkSimulationQuery(
//   userId: string,
//   messageId: string,
//   content: string,
...
//     /what if i (?:focus on|learn|study)/i,
//     /simulate (?:learning|studying|focusing on)/i,
//     /where will (?:learning|studying) (.*?) take me/i,
//     /compare learning (.*?) and (.*?)/i,
//   ];

//   if (!simulationPatterns.some(p => p.test(content))) {
//     return;
//   }

//   try {
//     const simulation = await this.extractSimulationParams(content);
//     if (simulation.path) {
//       const simulationResult = await this.futureSimulationService.simulatePath(
//         userId,
//         simulation.path,
...
//         duration: simulation.duration || 6,
//       };

//       // Only add fields that exist in your schema
//       if (simulationResult.primaryPath?.skillProjections) {
//         data.skillProjections = JSON.parse(JSON.stringify(simulationResult.primaryPath.skillProjections));
//       }

//       if (simulationResult.primaryPath?.marketOpportunities) {
//         data.marketOpportunities = JSON.parse(JSON.stringify(simulationResult.primaryPath.marketOpportunities));
//       }

//       if (simulationResult.primaryPath?.risks) {
//         data.risks = JSON.parse(JSON.stringify(simulationResult.primaryPath.risks));
//       }

//       if (simulationResult.alternatives) {
//         data.alternatives = JSON.parse(JSON.stringify(simulationResult.alternatives));
//       }

//       await this.prisma.assistantPathSimulation.create({ data });
//     }
//   } catch (error) {
//     this.logger.error('Simulation query check failed:', error);
//   }
// }
...
//           totalTokens: { increment: tokensUsed + this.estimateTokens(content) },
//           modeUsage: intent ? { [intent]: { increment: 1 } } : undefined,
//           updatedAt: new Date(),
//         },
//         create: {
//           userId,
//           totalMessages: 2,
//           totalTokens: tokensUsed + this.estimateTokens(content),
//           modeUsage: intent ? { [intent]: 1 } : {},
//         },
//       });
//     } catch (error) {
...
//    */
//   private async cacheArticleReferences(userId: string, content: string): Promise<void> {
//     try {
//       const articles = await this.extractReferencedArticles(content);
//       if (articles.length > 0) {
//         await this.cacheService.cacheArticleRecommendations(userId, articles);
//       }
//     } catch (error) {
//       this.logger.error('Article caching failed:', error);
//     }
//   }
...
//     userId: string,
//     userTier?: string,
//   ): Promise<{ remaining: number; limit: number; reset: number } | undefined> {
//     if (userTier !== 'FREE') return undefined;
...
//     const todayCount = cachedCount ? parseInt(cachedCount as string) || 0 : 0;
...
// private async extractDecisionOptions(content: string): Promise<{ optionA: string; optionB: string }> {
//   // Use AI to extract options from the query
//   const prompt = `Extract the two options being compared in this decision query: "${content}". Return as JSON with optionA and optionB.`;
...
//     }
//   } catch (error) {
//     this.logger.error('Failed to extract decision options:', error);
//   }
  
//   return { optionA: '', optionB: '' };
// }

// private formatDecisionResponse(originalContent: string, analysis: any): string {
//   return `${originalContent}\n\n## Decision Analysis\n\n${JSON.stringify(analysis, null, 2)}`;
// }

// private async extractSimulationParams(content: string): Promise<{ path: string; duration?: number }> {
//   const prompt = `Extract the learning path and duration from this query: "${content}". Return as JSON with path and optional duration in months.`;
...
//     }
//   } catch (error) {
//     this.logger.error('Failed to extract simulation params:', error);
//   }
  
//   return { path: '', duration: 6 };
// }

// private formatSimulationResponse(originalContent: string, simulation: any): string {
