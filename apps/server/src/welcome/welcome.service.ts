...
export class WelcomeService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log('Cverra Welcome Service initialized');
    ...
    try {
      ...
      await this.prisma.walletTransaction.create({
        data: {
          ...
          type: 'CREDIT',
          source: TransactionSource.BONUS,
          description: '🎉 Welcome bonus for joining Cverra!',
          metadata: { 
            type: 'WELCOME_BONUS', 
  ...
  private async sendWelcomeNotification(userId: string) {
    ...
    try {
      await this.sendCverraWelcome(userId, state.userName);
  ...
  // Send Cverra-specific welcome notification
  private async sendCverraWelcome(userId: string, userName: string) {
    try {
      await this.notificationService.createNotification(userId, 'SYSTEM', {
        title: '🎉 Welcome to Cverra!',
      ...
      this.logger.log(`Cverra welcome sent to ${userName} (${userId})`);
    } catch (error) {
      this.logger.error(`Error sending Cverra welcome:`, error);
    }
  }
  ...
  private async sendBonusAwardedNotification(userId: string) {
    try {
      await this.notificationService.createNotification(userId, 'SYSTEM', {
        ...
        message: 'You received 100 free coins to get started with Cverra! Use them to Explore powerful Cverra features.',
        metadata: {
          type: 'BONUS_AWARDED',
          coins: 100,
          action: 'VIEW_WALLET',
  ...
  private async sendFeatureIntroduction(userId: string, userName: string) {
    try {
      await this.notificationService.createNotification(userId, 'SYSTEM', {
        title: '✨ Meet Cverra\'s Core Features',
  ...
  private async sendTipsNotificationInternal(userId: string, userName: string) {
    try {
      await this.notificationService.createNotification(userId, 'SYSTEM', {
        ...
        message: `${userName}, here are quick tips to maximize your Cverra experience:`,
        metadata: {
          type: 'TIPS',
          tips: [
            'Create your first professional resume with AI power',
            'Craft smart letters for all situations with in seconds',
            'Explore Cverra deep knowledge Hub of articles',
